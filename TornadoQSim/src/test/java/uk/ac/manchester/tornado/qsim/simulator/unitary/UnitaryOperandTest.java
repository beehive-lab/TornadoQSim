...
public class UnitaryOperandTest {
    ...
    private void performMatrixProduct(ComplexTensor matrixA, ComplexTensor matrixB, ComplexTensor matrixC) {
        UnitaryOperand.matrixProduct(matrixA.getRawRealData(), matrixA.getRawImagData(), matrixA.shape()[0], matrixA.shape()[1], matrixB.getRawRealData(), matrixB.getRawImagData(), matrixB.shape()[1],
    ...
    private void performKroneckerProduct(ComplexTensor matrixA, ComplexTensor matrixB, ComplexTensor matrixC) {
        UnitaryOperand.kroneckerProduct(matrixA.getRawRealData(), matrixA.getRawImagData(), matrixA.shape()[0], matrixA.shape()[1], matrixB.getRawRealData(), matrixB.getRawImagData(),
                matrixB.shape()[0], matrixB.shape()[1], matrixC.getRawRealData(), matrixC.getRawImagData());
    }

    private void performBuildControlGate(ComplexTensor gate, int control, int target, ComplexTensor result) {
        UnitaryOperand.buildControlGate(gate.getRawRealData(), gate.getRawImagData(), control, target, result.getRawRealData(), result.getRawImagData(), result.shape()[0]);
    }
}
